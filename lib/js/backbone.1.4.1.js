...
(function (factory) {
    if (konsole.runtime)
        console.log(
            'I am the function that determines the parameters to pass into the Backbone factory function\n allowing one Backbone library to run in multiple environments (browser, AMD, node.js)'
        );
    var root =
        (typeof self == 'object' && self.self === self && self) ||
        (typeof global == 'object' && global.global === global && global);

    if (konsole.runtime) console.log('root', root);
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        if (konsole.runtime)
            console.log('Backbone.js is running in the browser');
    ...
    if (konsole.runtime) {
        console.log('I am a factory function, invoked with args:', {
            root,
            Backbone,
            _,
            $,
        });
        if (root) console.log('runtime:', root);
        if (Backbone)
            console.log(
                'Empty object on which to add all Backbone properties and methods'
            );
        if (_) console.log('Underscore.js is loaded');
        if ($) console.log('jQuery is loaded');
        if (root.Backbone)
            console.log(
                'There was a previous version of Backbone in this environment'
            );
    }
    ...
    //TODO: Start code dive with Backbone.Model
    ...
    _.extend(Collection.prototype, Events, {
        ...
        modelId: function (attrs, idAttribute) {
            return attrs[
                idAttribute || this.model.prototype.idAttribute || 'id'
            ];
        },
